 ...
 * - SOS Button                : GPIO12 (D6) [Active LOW with INPUT_PULLUP]
 * - Battery ADC               : A0     (0..3.3V ADC)
 * - Status LED                : GPIO2  (D4 Built-in LED Active LOW)
 * 
 * Features:
 * - Interactive Serial Monitor Text Messaging over 433MHz LoRa Mesh.
 * - Hardware SOS Push Button for immediate emergency broadcast.
 * - OLED display (128x64 I2C) showing incoming text messages, sender ID, RSSI, & SNR.
...
#define SOS_BUTTON_PIN  12   // D6 (GPIO12) Active LOW Emergency SOS Button
#define BATT_PIN        A0   // A0 Analog Battery Input
#define STATUS_LED_PIN  2    // D4 (GPIO2)  Built-in LED (Active LOW)
...
void updateOledDisplay() {
  ...
  if (lastRxAlertLevel > 0 && (millis() - lastRxTime < 30000)) {
    ...
    display.print(F("Status: READY (ESP8266)"));

    display.setCursor(0, 32);
    display.print(F("Serial: Type text"));
    display.setCursor(0, 42);
    display.print(F("Button: Press D6 SOS"));
...
void printSerialHelp() {
  ...
  Serial.println(F("  - Press physical D6 button to trigger Emergency SOS."));
...
void processIncomingPacket(int packetSize) {
  ...
  if (isForMe) {
    // Flash LED on receive (Active LOW LED on ESP8266)
    digitalWrite(STATUS_LED_PIN, LOW);
    delay(30);
    digitalWrite(STATUS_LED_PIN, HIGH);
